...
// Mengalokasikan memori bertipe compound page, dan mengembalikan alamat virtualnya.
...
// Melakukan pin (mengunci) halaman memori user dan mengembalikan daftar page.
...
// Membebaskan region peta memori yang telah dialokasikan.
...
// Menginisialisasi pointer ke memori dari region dengan vmapping jika perlu.
...
// Mem-pin halaman user space sebagai bagian dari region.
...
// Mengalokasikan halaman memori baru untuk region.
...
// Membuat region memori baru (baik user-provided atau dialokasikan kernel).
...
// Mengambil pointer ke region berdasarkan offset mmap.
...
// Validasi apakah mmap terhadap region valid dan dapat diakses.
...
// Validasi mmap berdasarkan file dan offset.
...
// Melakukan mapping halaman ke user space.
...
// Handler utama untuk mmap syscall pada io_uring file.
...
// Mengatur alamat mmap yang belum digunakan.
...
// Fallback mmap handler untuk sistem tanpa MMU.
int io_uring_mmap(struct file *file, struct vm_area_struct *vma)
{
	return is_nommu_shared_mapping(vma->vm_flags) ? 0 : -EINVAL;
}

// Mengembalikan kapabilitas mmap di sistem tanpa MMU.
unsigned int io_uring_nommu_mmap_capabilities(struct file *file)
{
	return NOMMU_MAP_DIRECT | NOMMU_MAP_READ | NOMMU_MAP_WRITE;
}

// Fallback get_unmapped_area di sistem tanpa MMU.
